...
function addToNgModule(tree: Tree, context: SchematicContext, filePath: string): Tree {
  ...
  if (!ngModuleDecoratorCall) {
    context.logger.warn(
      `[Component] No ${isModule ? '@NgModule' : '@Component'} decorator found. Skip.`,
    );
  ...
  if (!arg || !ts.isObjectLiteralExpression(arg)) {
    context.logger.warn(
      `[Component] Invalid ${isModule ? '@NgModule' : '@Component'} metadata. Skip.`,
    );
    tree.commitUpdate(recorder);
    return tree;
  }
  ...
  if (!importsProp || !ts.isArrayLiteralExpression(importsProp.initializer)) {
    context.logger.warn(
      `[Component] No imports[] found in ${isModule ? '@NgModule' : '@Component'} Skip.`,
    );
    tree.commitUpdate(recorder);
    return tree;
  }
