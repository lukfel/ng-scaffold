...
export class ScaffoldComponent {
  ...
  public headerConfig = computed<HeaderConfig | null>(
    () => this.scaffoldConfig()?.headerConfig || null,
  );
  public navbarConfig = computed<NavbarConfig | null>(
    () => this.scaffoldConfig()?.navbarConfig || null,
  );
  public drawerConfig = computed<DrawerConfig | null>(
    () => this.scaffoldConfig()?.drawerConfig || null,
  );
  public drawerPortal = toSignal<ComponentPortal<unknown> | TemplatePortal<unknown> | null>(
    this.scaffoldService.drawerPortal$,
  );
  public footerConfig = computed<FooterConfig | null>(
    () => this.scaffoldConfig()?.footerConfig || null,
  );
  public contentTitleCardConfig = computed<ContentTitleCardConfig | null>(
    () => this.scaffoldConfig()?.contentTitleCardConfig || null,
  );
  public floatingButtonConfig = computed<FloatingButtonConfig | null>(
    () => this.scaffoldConfig()?.floatingButtonConfig || null,
  );
  public bottomBarConfig = computed<BottomBarConfig | null>(
    () => this.scaffoldConfig()?.bottomBarConfig || null,
  );

  public routeHistory = toSignal<string[]>(
    this.routerService.routeHistory$.pipe(
      tap((routeHistory: string[]) => {
  ...
  public isMobile = toSignal<boolean>(
